...
for i in range(len(obs)):
    if len(obs[i]) == 4:
        ...
        stem=obs[i]
    print(obs[i])
    #if os.path.isfile(wd+'data/'+obs[i]+'/repro_new_asol/expo_broad_flux_4rebinned.img') == False:
    #    print('Creating img here.')
    ...
    s.call('dmcopy \"'+wd+'data/'+obs[i]+'/repro_new_asol/acisf'+stem+'_repro_evt2.fits[ccd_id=0:3][energy=500:7000]\" '+wd+'data/'+obs[i]+'/repro_new_asol/acisf'+stem+'_repro_05to7keV.fits clobber=yes',shell=True)
