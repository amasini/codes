...
cat1=fits.open(wd+'sim_all/mosaic_'+band+'_sim_poiss_cat1_3.fits')
...
#define cut at 3% of spurious fraction
if band=='broad':
	cut=1.4e-2
elif band=='soft':
	cut=1e-2
...
bins=np.logspace(np.log10(5e-17),np.log10(9e-14),50)
...
w2=open(wd+'sim_all/cdwfs_'+band+'_sim_poiss_matched.dat','w')
unmatched=0
blendings=0
cts=[]
		...
		if d <= match_rad: #found a match
			if found==0: #it's the first
				if flux_k[i]/flux_cdwfs[j] < 10.: #and the flux ratio is less than a factor of 10
					...
					w2.write(str(ra_k[i])+' \t '+str(dec_k[i])+' \t '+str(prob[i])+' \t '+str(flux_cdwfs[j])+' \t '+str(flux_k[i])+' \t '+str(d)+' \n')
					at_least_one=True
